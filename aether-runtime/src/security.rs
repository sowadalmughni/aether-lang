...
impl std::fmt::Display for SecurityError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            SecurityError::PromptInjectionDetected(msg) => {
                write!(f, "Prompt injection detected: {}", msg)
            }
...
impl DefaultInputSanitizer {
    pub fn new(config: SecurityConfig) -> Self {
        let blacklisted_patterns = config
...
impl InputSanitizer for DefaultInputSanitizer {
    async fn sanitize(&self, input: &str) -> Result<String, SecurityError> {
        ...
        if self.config.enable_prompt_injection_guard {
            for pattern in &self.blacklisted_patterns {
                if input_lower.contains(pattern) {
                    ...
                    return Err(SecurityError::PromptInjectionDetected(format!(
                        "Detected blacklisted pattern: {}",
                        pattern
                    )));
                }
            }
        ...
        if self.config.enable_profanity_filter {
            self.check_profanity(&input_lower).await?
        }

        info!(input_length = input.len(), "Input sanitization passed");
    ...
    async fn check_profanity(&self, input: &str) -> Result<(), SecurityError> {
        ...
        for word in profane_words {
            if input.contains(word) {
                ...
                return Err(SecurityError::ProfanityDetected(format!(
                    "Detected profane content: {}",
                    word
                )));
            }
        }
...
mod tests {
    ...
    async fn test_prompt_injection_detection() {
        ...
        assert!(matches!(
            result,
            Err(SecurityError::PromptInjectionDetected(_))
        ));
    }
